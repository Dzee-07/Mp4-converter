...
- **Search for YouTube** — fully implemented via the official YouTube Data
  API v3 (`GET /api/search?platform=youtube&q=...`). Needs a free API key:
  console.cloud.google.com → APIs & Services → enable "YouTube Data API v3"
  → Credentials → Create API key. Put it in `backend/.env` as
  `YOUTUBE_API_KEY=...`. Without it, search for YouTube returns a clear
  error instead of fake results.
- **Search for TikTok/Facebook/Instagram** — not implemented. None of them
  expose a public search API without an approved partner app, so
  `/api/search` returns `success: false` with an explanation for these
