    ...
    return (
      <div className="min-h-screen w-full flex flex-col items-center justify-center relative overflow-hidden" style={{ backgroundColor: "#0B0B0D" }}>
        <div className="flex flex-col items-center gap-5 animate-pulse">
          <OmegaLogo size={92} />
          <div className="text-white/90 font-semibold tracking-wide text-lg" style={{ fontFamily: "'Sora', sans-serif" }}>
            Omega Converter
          </div>
        </div>
    ...
    <div className={`min-h-screen w-full ${textCls} font-sans transition-colors duration-300`} style={{ fontFamily: "'Inter', sans-serif", colorScheme: dark ? "dark" : "light", backgroundColor: pageBgColor }}>
      ...
      <header className="max-w-3xl mx-auto px-4 sm:px-6 pt-6 pb-4 flex items-center justify-between">
        ...
        <div className="flex items-center gap-2.5">
          <OmegaLogo size={34} />
          <span className="font-semibold text-lg sm:text-xl tracking-tight" style={{ fontFamily: "'Sora', sans-serif" }}>
            Omega Converter
          </span>
        </div>
        ...
        {stage === "detail" && selected && (
          <div className="rounded-2xl border p-4 sm:p-5" style={{ backgroundColor: cardBgColor, borderColor: cardBorderColor }}>
            ...
            {theme_.watermark && (
              <button onClick={() => setRemoveWatermark((v) => !v)} className={`mt-5 w-full flex items-center justify-between rounded-xl border px-4 py-3 ${dark ? "border-white/10" : "border-black/10"}`}>
                <span className="flex items-center gap-2 text-sm">
                  <ShieldOff size={16} className={subtle} /> Remove watermark
                </span>
            ...
            <div className="mt-5 flex gap-3">
              ...
              <button onClick={resetFlow} className={`px-4 py-3 rounded-xl border text-sm ${dark ? "border-white/10 hover:bg-white/5" : "border-black/10 hover:bg-black/5"}`}>
                New
              </button>
            </div>
                ...
                <div key={h.id} className={`flex items-center justify-between rounded-xl border px-3 py-2.5 ${dark ? "border-white/10" : "border-black/10"}`}>
                  <div className="min-w-0">
                    ...
                    <div className={`text-xs ${subtle}`}>
                      {PLATFORMS[h.platform].label} - {h.resolution} - {h.time}
                    </div>
                  </div>
                </div>
              ))}
              <button onClick={() => setHistory([])} className="flex items-center gap-1.5 text-xs text-red-400 mt-2">
      ...
      {showProfile && (
        <Overlay dark={dark} onClose={() => { setShowProfile(false); setAuthMode(null); }}>
          <div className="flex items-center justify-between mb-4">
            ...
            <button onClick={() => { setShowProfile(false); setAuthMode(null); }}>
              <X size={20} />
            </button>
          </div>
          ...
          {account ? (
            <div className={`rounded-xl border px-4 py-3 mb-4 ${dark ? "border-white/10" : "border-black/10"}`}>
              ...
              <div className="flex items-center justify-between mt-3">
                <span className={`text-xs flex items-center gap-1 ${subtle}`}>
                  <Crown size={13} /> {PLANS_LABEL(account.plan)} plan
                </span>
              ...
              <button onClick={() => setAccount(null)} className="flex items-center gap-1.5 text-xs text-red-400 mt-3">
            ...
            <div className="flex flex-col gap-2 mb-4">
              ...
              <button onClick={() => setAuthMode("login")} className={`w-full py-2.5 rounded-xl border text-sm ${dark ? "border-white/10 hover:bg-white/5" : "border-black/10 hover:bg-black/5"}`}>
                Log in
              </button>
              <button onClick={() => setAuthMode("signup")} className={`w-full py-2.5 rounded-xl text-sm text-white bg-gradient-to-r ${theme_.gradient}`}>
                Create account
              </button>
            </div>
          )}
      ...
      {subscribeOpen && (
        <Overlay dark={dark} onClose={() => setSubscribeOpen(false)}>
          <div className="flex items-center justify-between mb-4">
            ...
            <button onClick={() => setSubscribeOpen(false)}>
              <X size={20} />
            </button>
          </div>
              ...
              <div key={p.id} className={`rounded-xl border p-4 ${dark ? "border-white/10" : "border-black/10"}`}>
                <div className="flex items-center justify-between">
                  ...
                  <div className="font-semibold text-sm">
                    {formatMoney(p.usd, currencyCode, locale)}
                    {p.usd > 0 ? "/mo" : ""}
                  </div>
                </div>
      ...
      {previewItem && (
        <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/55 p-4" onClick={closePreview}>
          <div onClick={(e) => e.stopPropagation()} className="relative w-full max-w-md rounded-2xl overflow-hidden shadow-2xl">
            <button onClick={closePreview} className="absolute top-3 right-3 z-10 w-8 h-8 rounded-full bg-black/60 flex items-center justify-center" aria-label="Close preview">
            ...
            <div className="p-4" style={{ backgroundColor: "#151517" }}>
              ...
              <button
                onClick={() => {
                  closePreview();
                  openDownloadFlow(previewItem);
                }}
                className={`mt-3 w-full py-2.5 rounded-xl text-sm font-medium text-white bg-gradient-to-r ${theme_.gradient}`}
              >
      ...
      <DownloadFlowSheet
        flow={downloadFlow}
        gradientClass={theme_.gradient}
        dark={dark}
        availableResolutions={RESOLUTIONS[platform]}
        onClose={() => setDownloadFlow(null)}
        onPickResolution={(r) => runDownload(downloadFlow.item, r)}
      />
...
function Overlay({ children, onClose, dark = true }) {
  return (
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center bg-black/50" onClick={onClose}>
      <div
        onClick={(e) => e.stopPropagation()}
        className="w-full sm:max-w-sm sm:rounded-2xl rounded-t-2xl p-5 max-h-[80vh] overflow-y-auto"
...
function AuthForm({ mode, dark, onCancel, onSubmit }) {
  ...
  return (
    <form
      onSubmit={(e) => {
        e.preventDefault();
        onSubmit({ name: name || "New user", email: email || "you@example.com" });
      }}
      className="flex flex-col mb-4"
    >
      {mode === "signup" && (
        <input className={inputCls} placeholder="Name" value={name} onChange={(e) => setName(e.target.value)} style={{ color: dark ? "#fff" : "#111" }} />
      )}
      ...
      <div className="flex gap-2 mt-1">
        <button type="button" onClick={onCancel} className={`flex-1 py-2.5 rounded-xl border text-sm ${dark ? "border-white/10" : "border-black/10"}`}>
          Cancel
        </button>
        <button type="submit" className="flex-1 py-2.5 rounded-xl text-sm text-white bg-gradient-to-r from-orange-500 to-pink-600">
...
function AdminLoginForm({ dark, onSubmit }) {
  ...
  return (
    <form
      onSubmit={(e) => {
        e.preventDefault();
        if (!onSubmit(username, password)) setError("Incorrect username or password.");
      }}
    ...
    >
      ...
      <button type="submit" className="w-full py-2.5 rounded-xl text-sm font-medium text-white bg-gradient-to-r from-orange-500 to-pink-600">
        Log in
      </button>
...
function Thumb({ item, gradientClass, size, onPreview }) {
  ...
  return (
    <button
      ...
      className={`relative rounded-xl overflow-hidden flex-shrink-0 ${dims} ${thumbUrl ? "bg-black" : `bg-gradient-to-br ${gradientClass}`}`}
    ...
    >
      ...
      {item.duration && (
        <span className="absolute bottom-1 right-1 px-1.5 py-0.5 rounded bg-black/80 text-white text-[10px] font-medium leading-none">{item.duration}</span>
      )}
    </button>
  );
}
  ...
  return (
    <div className="fixed inset-0 z-[65] flex items-end sm:items-center justify-center bg-black/55 p-0 sm:p-4" onClick={phase === "resolution" ? onClose : undefined}>
      <div
        ...
        style={{ backgroundColor: dark ? "#17171A" : "#FFFFFF", color: dark ? "#fff" : "#111318" }}
      >
        ...
        {phase === "failed" && (
          <div className="flex items-center gap-2 text-sm text-red-400 py-2">
            <AlertCircle size={16} /> Download failed - check your connection and try again.
          </div>
        )}
      </div>
    </div>
  ...
  return (
    <div className={`min-h-screen w-full ${textCls}`} style={{ backgroundColor: pageBgColor, colorScheme: dark ? "dark" : "light" }}>
      <header className="max-w-3xl mx-auto px-4 sm:px-6 pt-6 pb-4 flex items-center justify-between">
        ...
        <button onClick={onLogout} className={`flex items-center gap-1.5 text-xs ${subtle}`}>
          <LogOut size={13} /> Log out
        </button>
      </header>
      ...
      <div className="max-w-3xl mx-auto px-4 sm:px-6 pb-16">
        {!selectedUser ? (
          <div>
            <div className={`flex items-center gap-2 text-xs uppercase tracking-wide mb-3 ${subtle}`}>
              <UsersIcon size={13} /> {users.length} users
            </div>
            <div className="flex flex-col gap-2">
              {users.map((u) => (
            ...
            <div className="rounded-2xl border p-4 mb-4" style={{ backgroundColor: cardBgColor, borderColor: cardBorderColor }}>
              ...
              <div className="flex items-center gap-4 mt-3 text-xs">
                <span className="flex items-center gap-1">
                  <Crown size={13} /> {PLANS_LABEL(selectedUser.plan)} plan
                </span>
                <span className="flex items-center gap-1">
                  <Clock size={13} /> Joined {new Date(selectedUser.joinedAt).toLocaleString()}
                </span>
              </div>
            </div>

            <div className={`text-xs uppercase tracking-wide mb-2 flex items-center gap-1.5 ${subtle}`}>
                ...
                {selectedUser.downloads.map((d) => (
                  <div key={d.id} className="flex items-center justify-between rounded-xl border px-3 py-2.5" style={{ backgroundColor: cardBgColor, borderColor: cardBorderColor }}>
                    <div className="min-w-0">
                      ...
                      <div className={`text-xs ${subtle}`}>
                        {PLATFORMS[d.platform]?.label} - {d.resolution} - {d.time}
                      </div>
                    </div>
                  </div>
                ))}
